...
| `checksum_start` | int | 是 | **校验计算起始位置**（包含，从0开始） | `0` |
| `checksum_end` | int | 是 | **校验计算结束位置**（不包含，Python切片语义） | `104` |

> **重要说明**：`checksum_end` 使用 Python 切片语义（左闭右开）
> - "从 0 到 104" 表示 `data[0:104]`，实际计算索引 0~103（共104个字节）
> - 索引104的字节（校验码本身）不参与计算
...
#### checksum_end（校验结束位置）⚠️
- **含义**: 计算到第几个字节（**不包含**此位置，Python切片语义）
- **常见值**: 校验码的位置（不包含校验码本身）
- **你的协议**: `"checksum_end": 104`（到校验码前）

> **⚠️ 重要理解**：
> - `checksum_end: 104` 表示计算 `data[0:104]`
> - 实际计算：索引 0, 1, 2, ... 103（共104个字节）
> - **不包含**索引104（校验码本身）
...
```json
{
  "checksum_start": 0,    // 从位置0开始
  "checksum_end": 104     // 到位置104（不含）
}
```
