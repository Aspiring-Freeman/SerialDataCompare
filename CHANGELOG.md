...
#### 1. HEX日志提取器增强
- **新增短横线分隔格式支持**：支持从 CNGas 等日志系统提取 HEX 数据
  - 格式示例：`发送的指令：68-AA-AA-AA-AA-AA-AA-68-05-12-00-D1-16`
- **新增冒号分隔格式支持**：支持 MAC 地址风格的 HEX 数据
  - 格式示例：`68:AA:AA:AA:AA:AA:68:05:12:D1:16`
- **中英文冒号兼容**：同时支持中文冒号（：）和英文冒号（:）
- **更新提示信息**：占位符文本现在展示所有支持的格式
...
#### 1. HEX日志提取器小工具
- **新增工具**：在小工具界面添加“HEX日志提取”工具
- **功能说明**：从串口调试日志中提取HEX数据帧
- **支持格式**：
  - DGM_RX/DGM_TX 格式：`[12:13:39.223] RX ← D/HEX DGM_RX: 0000-0007: 68 01 80 16...`
  - 带地址范围的十六进制日志格式
- **输出格式**：
  - HEX数据（空格分隔，每行16字节）
  - HEX数据（无分隔，连续字符串）
